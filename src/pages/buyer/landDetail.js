...
import BuyerLandIcon from "../../assets/buyer-land.svg";
...
export default function BuyerLandDetail({ setTabSelected, tabSelected }) {
  ...
  const fetchLandDetail = async () => {
    try {
      const result = await (
        await request.get(`api/v1/my_bids/${params?.bid_id}/`)
      ).json();
      setLand(result);
      scrollToBottom();
    } catch (error) {
  ...
  return (
    <>
      <div className="buyer-container">
        <Header />
        <section className="buyerinner" style={{paddingBottom:"0px"}}>
          ...
          <div className="container mt-5" style={{zIndex: "3",position: "relative"}}>
            <div className="topaction" onClick={() => history.push("/buy/2")}>
              <img src={LeftIcon} alt="" />
              <strong>Back to Bids</strong>
            </div>
            <section className="ld-card darkcards">
                  ...
                  >
                    <div className="row">
                      ...
                      <div className="col-lg-8 col-md-8 col-sm-8 land-detail">
                        <div>
                          <h3 className="ld-title text-white mt-3 mb-0">
                            {land?.name || "Land Title Goes Here"}
                            {/* <img
                              src={land?.user_avatar || UserNewIcon}
                              className="float-right ld-user-img img-fluid"
                              alt=""
                              style={{width: "40px", height: "40px"}}
                            /> */}
                            {/* <span className="mt-2 mr-3 float-right" style={{fontWeight: "bold",
                            fontSize: "16px",
                            lineHeight: "24px",
                            color: "#D8D8D8",
                            opacity: "0.5"}}>{"OWNER"}</span> */}
                          </h3>
                        </div>
                        <div style={{minHeight:"60px"}}>
                          <div className="ld-b-desc">
                            {land?.description || "Lorem ipsum dolor sit amet, consectetur adipiscing elit."}
                          </div>
                        </div>
                        <div className="mt-4 mb-1 d-flex align-items-center">
                          <span className="padding-right-one">
                            <span className="text-label">{"Seller price range"}</span>
                              {/* ${inputError ? "inputError" : ""} */}
                          </span>
                            <div className="input-group mb-2 opacity-50" style={{width:"auto"}}>
                            ...
                            <span className="ld-hypen ml-3 mr-3 opacity-50">{"-"}</span>
                            <div className="input-group mb-2 opacity-50" style={{width:"auto"}}>
                              <input className={`ld-input form-control `} 
                                type="number"
                                style={{ backgroundColor: "#425876", width: "140px",
                            ...
                            <span className="ld-price-text ml-3 opacity-50">{"MANA"}</span>
                        </div>
                        <div className="mt-4 mb-1 d-flex align-items-center">
                          <span className="padding-right-one mr-2">
                            <span className="text-label">{"Bid Price Offered"}</span>
                              {/* ${inputError ? "inputError" : ""} */}
                          </span>
                        ...
                        <div className="info-text text-center">
                          {"*Click on counter to edit or update the bid price"}
                        </div>
                        <hr style={{background:"rgba(255, 255, 255, 0.32)"}} />
                        <div className="mt-4 mb-4">
                          <span className="text-label text-yellow padding-right-two borderRight">
                            {"Position"} &nbsp; &nbsp; 
                            <span className="text-white" style={{fontWeight:"600"}}>
                              {land?.x || ""}, {land?.y || ""}
                            </span>
                          </span>
                          <span className="text-label text-yellow padding-right-two padding-left-two">
                            {"Type"} &nbsp; &nbsp;
                            <span className="text-white" style={{fontWeight:"600"}}>
                          ...
                          {/* <span className="padding-left-two">
                            <button className="btn btn-save-buy mr-5" type="button" onClick={handleSave}>{"Save Changes"}</button>
                            <button className="btn btn-remove" type="button" onClick={handleRemove}>{"Remove Bid"}</button>
                          </span> */}
                        </div>
                      </div>
                    </div>
              ...
              <hr style={{background:"rgba(255, 255, 255, 0.32)", margin: "40px 0px"}} />
                    ...
                    <div className="input-group">
                      ...
                      <div className="input-group-append">
                        <span className="input-group-text ld-chat-input-suffix" id="basic-addon1">
                          ...
                          <button style={{background: "#0e2440",
    border: "none"}} type="button" onClick={handleSendMessage}><img  className="mr-5" src={SendIcon} alt=""/></button>
                        </span>
                      </div>
                    </div>
                    </div>
